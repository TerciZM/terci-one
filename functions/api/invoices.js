...
const validStatuses = new Set(["Draft","Sent","Partial","Paid","Overdue","Void"]);
...
function clean(body) {
  ...
  const subtotal=lines.reduce((sum,line)=>sum+line.quantity*line.rate,0),discount=Math.max(0,Number(body.discount||0)),adjustment=Number(body.adjustment||0),total=Math.max(0,subtotal-discount+adjustment),amountPaid=Math.max(0,Number(body.amount_paid||0));
  return {...body,lines,subtotal,discount,adjustment,total,amount_paid:amountPaid,balance_due:Math.max(0,total-amountPaid)};
}
...
export async function onRequestPost({env,request}) {
  try {
    ...
    if(id){
      ...
      await env.DB.prepare("UPDATE invoices SET invoice_number=?,customer_id=?,order_number=?,invoice_date=?,payment_terms=?,due_date=?,salesperson=?,subject=?,status=?,subtotal=?,discount=?,adjustment=?,total=?,amount_paid=?,balance_due=?,customer_notes=?,terms_conditions=?,updated_at=CURRENT_TIMESTAMP WHERE id=?")
        .bind(invoiceNumber,customerId,body.order_number||null,invoiceDate,body.payment_terms||"Due on Receipt",dueDate,body.salesperson||null,body.subject||null,validStatuses.has(body.status)?body.status:"Draft",body.subtotal,body.discount,body.adjustment,body.total,body.amount_paid,body.balance_due,body.customer_notes||null,body.terms_conditions||null,id).run();
    ...
    if(!invoiceNumber||duplicate)invoiceNumber=await nextNumber(env,invoiceDate);
    const result=await env.DB.prepare("INSERT INTO invoices (invoice_number,customer_id,order_number,invoice_date,payment_terms,due_date,salesperson,subject,status,subtotal,discount,adjustment,total,amount_paid,balance_due,customer_notes,terms_conditions) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
      .bind(invoiceNumber,customerId,body.order_number||null,invoiceDate,body.payment_terms||"Due on Receipt",dueDate,body.salesperson||null,body.subject||null,validStatuses.has(body.status)?body.status:"Draft",body.subtotal,body.discount,body.adjustment,body.total,body.amount_paid,body.balance_due,body.customer_notes||null,body.terms_conditions||null).run();
